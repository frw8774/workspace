...
int input_jobs(Job jobs[]) {
    ...
    // 合法性校验，默认生成3个测试作业
    if (n < 1 || n > MAX_JOB) {
        printf("作业数量无效,自动生成3个测试作业\n");
        n = 3;
        // 作业1：0时刻到达，512MB内存、1台打印机、1台磁盘，运行5时间单位
        jobs[0] = (Job){1, 0, {512, 1, 1}, 5, 0, 0, 0.0, 0.0, 0};
        // 作业2：1时刻到达，256MB内存、1台打印机、1台磁盘，运行3时间单位
        jobs[1] = (Job){2, 1, {256, 1, 1}, 3, 0, 0, 0.0, 0.0, 0};
        // 作业3：2时刻到达，128MB内存、0台打印机、1台磁盘，运行4时间单位
        jobs[2] = (Job){3, 2, {128, 0, 1}, 4, 0, 0, 0.0, 0.0, 0};
    } else {
        // 手动输入作业信息
        for (int i = 0; i < n; i++) {
            printf("=== 输入第%d个作业信息 ===\n", i+1);
    ...
    // 输出作业清单
    printf("=== 作业信息清单 ===\n");
    printf("作业ID\t到达时间\t内存需求\t打印机需求\t磁盘需求\t运行时间\n");
    for (int i = 0; i < n; i++) {
        printf("%d\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
               jobs[i].id, jobs[i].arrive_time, jobs[i].need_res[0],
               jobs[i].need_res[1], jobs[i].need_res[2], jobs[i].run_time);
    }
    printf("============================\n");
    return n;
}

// 按FCFS规则排序作业（先到达时间，后作业ID）
void sort_jobs_by_fcfs(Job jobs[], int n) {
    Job temp;
    for (int i = 0; i < n-1; i++) {
        for (int j = 0; j < n-i-1; j++) {
            // 到达时间晚的排后面；到达时间相同，ID大的排后面
            if (jobs[j].arrive_time > jobs[j+1].arrive_time ||
                (jobs[j].arrive_time == jobs[j+1].arrive_time && jobs[j].id > jobs[j+1].id)) {
                temp = jobs[j];
                jobs[j] = jobs[j+1];
                jobs[j+1] = temp;
            }
    ...
    printf("作业已按先来先服务(FCFS)调度算法完成排序\n");
    printf("============================\n");
}
...
void job_schedule(Job jobs[], int n, SystemResource *sys_res) {
    ...
    while (completed_job < n) {
        ...
        // 输出作业调度详情
        printf("作业%d调度详情:\n", finished_job->id);
        printf("到达时间:%d | 开始时间:%d | 完成时间:%d\n",
               finished_job->arrive_time, finished_job->start_time, finished_job->finish_time);
...
void output_statistics(Job jobs[], int n) {
    ...
    printf("=== 作业调度统计总表 ===\n");
    printf("作业ID\t到达时间\t开始时间\t完成时间\t周转时间\t带权周转时间\n");
    for (int i = 0; i < n; i++) {
        printf("%d\t%d\t\t%d\t\t%d\t\t%.1f\t\t%.2f\n",
               jobs[i].id, jobs[i].arrive_time, jobs[i].start_time,
...
int main() {
    ...
    // FCFS排序
    sort_jobs_by_fcfs(jobs, job_num);
